data "local_file" "cloudwatch_agent_script" {
  filename = "${file("${path.module}/files/userhost/cloudwatch_agent.sh")}"
}
...
data "local_file" "userhost_config_hcs_script" {
  filename = "${file("${path.module}/files/userhost/userhost_config_hcs.sh")}"
}
...
data "local_file" "userhost_logging_script" {
  filename = "${file("${path.module}/files/userhost/userhost_logging.sh")}"
}
...
data "local_file" "userhost_logrotate_script" {
  filename = "${file("${path.module}/files/userhost/userhost.logrotate")}"
}
...
data "local_file" "sysdig_service" {
  filename = "${file("${path.module}/audit/sysdig.service")}" 
}
...
data "local_file" "json_lua" {
  filename = "${file("${path.module}/audit/json.lua")}"
}
...
data "local_file" "spylog_lua" {
  filename = "${file("${path.module}/audit/spy_log.lua")}"
}
...
data "local_file" "ecs_healthcheck" {
  filename = "${file("${path.module}/scripts/ecs_instance_health_check.py")}"
}
